...
# 获取项目根目录（假设clip_modules在项目根目录下）
project_root = os.path.dirname(current_dir)  # 如果clip_modules在上一级目录
# 或者直接指定clip_modules的路径
# project_root = "/path/to/your/project"  # 替换为实际路径
...
def get_pair_scores_objs(attr, obj, all_objs, attrs_by_obj_train, obj_embedding_sim):
    score = -1.
    # 遍历所有的物体找出所有物体和给的属性是否在训练集有组合过， 然后计算给定的物体和该物体的相似度，这里找的是最大的相似度
    for o in all_objs:
